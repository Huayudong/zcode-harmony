...
export class TopicWireFrameAssembler<F> {
  ...
  constructor(
    private readonly frameSchema: any,
  ...
  accept(wire: TopicWireFrameCandidate, now = Date.now()): TopicWireAssemblyEvent<F>[] {
    ...
    let json: string;
    try {
      // ArkTS 默认 lib 的 TextDecoder 构造签名不可用（0 参），运行时行为不变：严格 UTF-8 解码
      const DecoderCtor = TextDecoder as unknown as new (encoding?: string, options?: object) => { decode(input: Uint8Array): string };
      json = new DecoderCtor("utf-8", { fatal: true }).decode(logical);
    } catch {
      this.settle(key, assembly);
