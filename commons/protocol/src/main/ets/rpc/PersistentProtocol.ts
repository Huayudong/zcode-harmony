/**
 * PersistentProtocol —— 可靠性层（@zcode/rpc persistent-protocol.ts 的鸿蒙移植，L1 收尾）。
 *
 ...
 *   （帧级对事件级的唯一新增接口——事件级据此暂停 drain，丢弃/降级永远发生在
 *   通道层、进 rpc 之前；接了 send() 的帧绝不丢）
 *
 * 关键：Channel RPC 层完全不知道 PersistentProtocol 的存在，
 * 它只看到 IMessagePassingProtocol 接口。
 */
...
export class PersistentProtocol implements IMessagePassingProtocol, ConnectionFlowControl {
  ...
  private bindSocket(): void {
    this.disposables.add(
      this.socket.onData((data: VSBuffer) => {
